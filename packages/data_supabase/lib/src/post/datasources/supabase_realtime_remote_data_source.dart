...
class SupabaseRealtimeRemoteDataSource implements RealtimeRemoteDataSource {
  SupabaseRealtimeRemoteDataSource({required this._supabaseClient}) {
  ...
  void _handleAuthStateChange(AuthState authState) {
    ...
    if ((event == AuthChangeEvent.signedIn ||
      ...
      if (!_isSubscribed) {
        print(
          'Event: InitialSession. Channel not subscribed, initial connection...',
        );
        connect();
      }
    ...
    _channel!
        ...
        .subscribe((RealtimeSubscribeStatus status, Object? error) {
          if (status == RealtimeSubscribeStatus.subscribed) {
            print('Realtime Channel Subcribed Successfully');
            _isSubscribed = true;
